...
// POST /api/explain
// Streamt eine AI-Erklärung zur Frage: warum die richtige Antwort richtig ist
// und wo die Antwort des Users ggf. daneben lag.
export async function POST(req: Request) {
  ...
  // RAG: passende Lerninhalt-Chunks als Kontext holen (leer, wenn keine
  // Embeddings erzeugt wurden oder OPENAI_API_KEY fehlt — dann geht es ohne).
  ...
  const result = streamText({
    ...
    prompt: [
      ...
      question.explanation ? `Hinterlegte Kurz-Erklärung: ${question.explanation}` : "",
  ...
  // Quellen als Header mitgeben (base64-codiert), damit die UI Links rendern kann.
  return result.toTextStreamResponse({
    headers: {
      "x-sources": Buffer.from(JSON.stringify(sources), "utf8").toString(
        "base64"
      ),
    },
  });
