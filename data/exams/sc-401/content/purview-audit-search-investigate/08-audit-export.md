---
title: "Analyze and report on audit results"
url: "https://learn.microsoft.com/en-us/training/modules/purview-audit-search-investigate/audit-export"
uid: "learn.wwl.purview-audit-search-investigate.audit-export"
...
2.  On the search results page, select **Export** to download the audit records to a CSV file.
    ...
    ![Screenshot showing where to select Export to export data.](../../wwl-sci/purview-audit-search-investigate/media/audit-export-audit-search-results.png)
...
2.  Right-click the **AuditData** column, choose **Transform** > **JSON**, then use the expand icon to promote the JSON properties into columns.
3.  Deselect properties you don't need, then **Close & Load** to return the flattened data to Excel.

For the full walkthrough with screenshots, see [Export, configure, and view audit log records](/en-us/purview/audit-log-export-records?azure-portal=true).
...
2.  Filter the `MailItemsAccessed` export to rows for the same users and time window, expanding `Folders[].FolderItems[]` down to individual messages.
3.  Join the two filtered tables on user, resource identifier, and a time window loose enough to catch records that landed within a few minutes of each other.
...
`Search-UnifiedAuditLog` returns a maximum of 5,000 rows per call and 100 by default. The one-shot examples above silently truncate at 5,000 rows on larger date ranges. For larger exports, page through results with the `-SessionId` and `-SessionCommand ReturnLargeSet` parameters. See the [`Search-UnifiedAuditLog` reference](/en-us/powershell/module/exchange/search-unifiedauditlog?azure-portal=true) for the pagination pattern.
...
The healthcare investigation returns roughly 200,000 `MailItemsAccessed` records the team needs to correlate with Microsoft Entra sign-in logs in Excel. The tenant has Audit (Premium). What's the practical export path?

Export from the portal. A single Audit (Premium) export supports up to 1,000,000 rows.

Split the search into four smaller portal exports because a single portal export caps at 50,000 rows regardless of tier.

Use `Search-UnifiedAuditLog` in PowerShell only. Portal exports are limited to 5,000 rows for all tenants.
