---
title: "Configure and manage Microsoft Purview Audit"
url: "https://learn.microsoft.com/en-us/training/modules/purview-audit-search-investigate/audit-configure-manage"
uid: "learn.wwl.purview-audit-search-investigate.audit-configure-manage"
...
2.  On the user properties page, select **Licenses and apps**. Verify the user is licensed for Audit (Premium).
3.  In **Apps**, make sure **Microsoft 365 Advanced Auditing** is selected. If not, select it and save changes.
...
The healthcare compliance team just upgraded to Microsoft 365 E5 to get access to `MailItemsAccessed`. A week later, they search for the physician's message-level access and get no records. What's the most likely cause?

The upgrade is too recent. Audit (Premium) records take 30 days to start populating after a license change.

`MailItemsAccessed` requires **Microsoft 365 Advanced Auditing** to be enabled per user, and the E5 upgrade alone doesn't turn it on automatically.

The team needs to purchase the **10-Year Audit Log Retention** add-on before Premium events start logging.
