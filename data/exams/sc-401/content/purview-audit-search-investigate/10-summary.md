---
title: "Summary"
url: "https://learn.microsoft.com/en-us/training/modules/purview-audit-search-investigate/summary"
uid: "learn.wwl.purview-audit-search-investigate.summary"
...
Who deleted specific email messages

Record type `ExchangeItem`, operations `HardDelete`, `SoftDelete`, `MoveToDeletedItems`

Which files a user shared externally

Record type `SharePointSharingOperation`, operations like `SharingSet`, `AnonymousLinkCreated`

Which files a user accessed or downloaded

Record type `SharePointFileOperation`, operations `FileAccessed`, `FileDownloaded`

Which specific messages a user opened, when your tenant has Audit (Premium)

Operation `MailItemsAccessed` scoped to the user's mailbox

Whether a user used first-party Copilot

Record type `CopilotInteraction`

Whether a user used a Copilot Studio agent or Entra-registered AI app

Record type `ConnectedAIAppInteraction`. Requires DSPM onboarding.

Whether a user used a non-Microsoft AI website

Record type `AIAppInteraction`. Requires Purview Browser Extension plus DLP.

Admin permission or configuration changes

Record type `AzureActiveDirectory` and service-specific admin record types
...
2.  You have the **Audit Reader** or **Audit Manager** role in the Microsoft Purview portal.
3.  For any Audit (Premium) event, **Microsoft 365 Advanced Auditing** is enabled on the user whose activity you're investigating, and was enabled during the time window you care about.

## Resources

*   [Learn about auditing solutions in Microsoft Purview](/en-us/purview/audit-solutions-overview?azure-portal=true)
*   [Get started with auditing solutions](/en-us/purview/audit-get-started?azure-portal=true)
*   [Search the audit log](/en-us/purview/audit-search?azure-portal=true)
*   [Manage audit log retention policies](/en-us/purview/audit-log-retention-policies?azure-portal=true)
*   [Export, configure, and view audit log records](/en-us/purview/audit-log-export-records?azure-portal=true)
*   [Turn auditing on or off](/en-us/purview/audit-log-enable-disable?azure-portal=true)

## Use cases and scenarios

Explore how Microsoft Purview Audit can help investigate specific scenarios:

*   [Search the audit log to investigate common support issues](/en-us/purview/audit-troubleshooting-scenarios?azure-portal=true)
*   [Search the audit log for events in Microsoft Teams](/en-us/purview/audit-teams-audit-log-events?azure-portal=true)
*   [Use sharing auditing in the audit log](/en-us/purview/audit-log-sharing?azure-portal=true)
*   [Use Microsoft Purview Audit (Premium) to investigate compromised accounts](/en-us/purview/audit-log-investigate-accounts?azure-portal=true)
*   [Use a PowerShell script to search the audit log](/en-us/purview/audit-log-search-script?azure-portal=true)
