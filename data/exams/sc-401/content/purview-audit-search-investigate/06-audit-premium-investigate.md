---
title: "Investigate activities with Audit (Premium)"
url: "https://learn.microsoft.com/en-us/training/modules/purview-audit-search-investigate/audit-premium-investigate"
uid: "learn.wwl.purview-audit-search-investigate.audit-premium-investigate"
...
2.  **Slice the results to answer specific questions**:
...
3.  **Contextual analysis of access**:
...
4.  **Forensic recovery and reporting**:
...
For the full list of properties that break the deduplication window, see [Filter duplicate audit records](/en-us/purview/audit-log-investigate-accounts?azure-portal=true#filter-duplicate-audit-records).
...
During the physician's `MailItemsAccessed` investigation, several records show `IsThrottled` set to `True` inside the incident window. The team still needs to know which specific messages were opened during those hours. What's the practical next step?

Wait 24 hours and rerun the search. Throttled bind events backfill after the throttling window ends.

Escalate to Microsoft support. Throttled `MailItemsAccessed` records are permanently lost with no recovery option.

Cross-reference the gap with **Exchange message trace** or **Microsoft Entra sign-in logs**. Throttled bind events don't backfill, so gaps have to be reconstructed from other sources.
