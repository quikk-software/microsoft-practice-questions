---
title: "Search for Copilot and AI app activities"
url: "https://learn.microsoft.com/en-us/training/modules/purview-audit-search-investigate/audit-copilot"
uid: "learn.wwl.purview-audit-search-investigate.audit-copilot"
...
First-party Microsoft Copilot experiences, such as Microsoft 365 Copilot, Copilot Chat, Security Copilot, and Copilot in Fabric.

`ConnectedAIAppInteraction`

`ConnectedAIApp`

Custom Copilots built with Microsoft Copilot Studio and non-Microsoft AI apps registered in your tenant.

`AIAppInteraction`

`AIApp`

Non-Microsoft AI apps used outside your tenant, captured through browser or network data loss prevention (DLP).
...
`AIAppInteraction` records, and the non-Microsoft Entra-registered app scenarios under `ConnectedAIAppInteraction`, are billed through Microsoft Purview [pay-as-you-go billing](/en-us/purview/purview-billing-models?azure-portal=true). The tenant needs to be linked to an active Azure subscription and pay-as-you-go features have to be enabled before those records start ingesting. Copilot Studio agents are excluded.

For the setup steps, see [Use Microsoft Purview to manage data security and compliance for Entra-registered AI apps](/en-us/purview/ai-entra-registered?azure-portal=true) and [Use Microsoft Purview to manage data security and compliance for other AI apps](/en-us/purview/ai-other-apps?azure-portal=true).
...
*   **Where was the user when they interacted?** `AppHost`:
    ...
    Microsoft 365 Copilot Chat, Teams, or `microsoft365.com/copilot`
    
    `Bing`
    
    Microsoft Edge browser or Office mobile
    
    `Office`
    
    `office.com` or `microsoft365.com`
    
    `Word`, `Excel`, `PowerPoint`, `Stream`
...
*   **Activities - friendly names**: expand **Copilot activities**, or type _Copilot_ in the search box.
    ...
    [![Screenshot showing Interacted with Copilot selected under Activities - friendly names.](../../wwl-sci/purview-audit-search-investigate/media/audit-copilot-new-search-activities.png)](../../wwl-sci/purview-audit-search-investigate/media/audit-copilot-new-search-activities.png#lightbox)
...
*   **Record types**: select `CopilotInteraction`, `ConnectedAIAppInteraction`, or `AIAppInteraction`. Type _Copilot_ or _AI_ in the search box to filter the list.
    ...
    [![Screenshot showing CopilotInteraction selected under Record types.](../../wwl-sci/purview-audit-search-investigate/media/audit-copilot-new-search-record-type.png)](../../wwl-sci/purview-audit-search-investigate/media/audit-copilot-new-search-record-type.png#lightbox)
...
The healthcare compliance team searches audit for `ConnectedAIAppInteraction` records from a Copilot Studio agent that the physician used to summarize patient data. The search returns nothing, even though the physician confirmed using the agent that week. What's the most likely cause?

The physician needs a Microsoft 365 Copilot license. Copilot Studio agents require it to log audit records.

The agent hasn't been onboarded through Data Security Posture Management (DSPM). Copilot Studio agents only produce `ConnectedAIAppInteraction` records once they're onboarded.

The team needs to filter on `AIAppInteraction` instead, because Copilot Studio agents log under that record type.
