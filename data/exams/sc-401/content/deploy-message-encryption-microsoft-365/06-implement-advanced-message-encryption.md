---
title: "Control encrypted email access with Advanced Message Encryption"
url: "https://learn.microsoft.com/en-us/training/modules/deploy-message-encryption-microsoft-365/implement-advanced-message-encryption"
uid: "learn.wwl.deploy-message-encryption-microsoft-365.implement-advanced-message-encryption"
...
1.  Connect to [Exchange Online PowerShell](/en-us/powershell/exchange/connect-to-exchange-online-powershell?azure-portal=true).
    
2.  Run the following command to create a custom branding template that includes an expiration policy:
...
2.  Open the encrypted message.
3.  If the message is eligible for revocation, you'll see a **Remove external access** option at the top of the message. Select it to revoke access.
...
2.  Enable tracking logs for the encrypted message portal:
    
    ```
    Set-IrmConfiguration -EnablePortalTrackingLogs $true
    ```
    
3.  In the [Microsoft Purview portal](https://purview.microsoft.com?azure-portal=true), go to the **Audit** solution.
    
4.  Under **Search**, select **Encrypted message portal activities**, apply filters, and run your search.
