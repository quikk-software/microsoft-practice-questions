---
title: "Create and manage exact data match sensitive info types"
url: "https://learn.microsoft.com/en-us/training/modules/create-manage-sensitive-information-types/4-describe-custom-with-exact-data-match"
uid: "learn.wwl.create-manage-sensitive-information-types.describe-custom-with-exact-data-match"
...
Before you start creating EDM SITs, keep in mind that you need Global Admin, Compliance Admin, or Exchange Admin permissions. EDM is also only available in [specific regions](/en-us/microsoft-365/enterprise/o365-data-locations?azure-portal=true), so confirm your tenant's data residency before planning a deployment.

[![Diagram showing how exact data match based classification works.](../../wwl-sci/create-manage-sensitive-information-types/media/exact-data-match-classification.png)](../../wwl-sci/create-manage-sensitive-information-types/media/exact-data-match-classification.png#lightbox)
...
2.  **Managing more than 10 SITs**: If you need to create or manage more than 10 SITs, you need to use the classic experience. Because you can map multiple EDM SITs to the same schema, you can have more than 10 EDM SITs. Attempting to create an eleventh schema with the new experience generates an error.
3.  **Custom schema names**: The classic experience lets you specify custom names for your EDM schemas, unlike the new experience that autogenerates schema names.
4.  **Editing existing schemas**: If you need to edit schemas created in the classic experience or uploaded via PowerShell, you must use the classic experience, as the new experience doesn't support this functionality.
...
*   [New EDM experience](#tabpanel_1_new)
*   [Classic EDM experience](#tabpanel_1_classic)
...
2.  Make sure the **New EDM experience** toggle is set to **On**.
    ...
    [![Screenshot showing the toggle to use the new EDM experience.](../../wwl-sci/create-manage-sensitive-information-types/media/new-edm-toggle.png)](../../wwl-sci/create-manage-sensitive-information-types/media/new-edm-toggle.png#lightbox)
    
3.  Select **Create EDM classifier**.
    
4.  Review the **Familiarize yourself with the steps needed to put your classifier to work** page, then select **Create EDM classifier**.
    
5.  On the **Name and describe your EDM classifier** page, name the SIT, and add a description. The system uses this name, appended with the word _schema_, for the associated schema it generates.
    
6.  Select **Next**.
    
7.  On the **Choose a method for defining your schema** page, select the method you want to use for your schema: either **Upload a file containing sample data**, or **Manually define your data structure**.
    
    Best practice is to upload a sample data file. The rest of this procedure assumes this option.
    
8.  Select **Next**.
    
9.  On the **Upload your sample file** page, select your sample file, and then select **Upload file**. Select **Next**.
    
    If errors display during the upload, address them and then try again.
    
10.  On the **Select primary elements** page:
...
12.  **Configure settings for data in selected columns**.
...
13.  On the **Review settings and finish** page, select **Submit**.
     
14.  On the **You successfully created an EDM classifier** page, capture the **Schema name**. This name is required when hashing and uploading the sensitive information source table to ensure proper mapping of the data to the schema.
     
15.  Once you've captured the schema name, select **Done**.
     

Once you create your EDM schema, the next step is to hash and upload your sensitive data. This ensures the data can be used securely for classification. For detailed steps on hashing and uploading your source table, see [Hash and upload the sensitive information source table for exact data match sensitive information types](/en-us/purview/sit-get-started-exact-data-match-hash-upload?azure-portal=true)

1.  Sign in to the [Microsoft Purview portal](https://purview.microsoft.com?azure-portal=true), then navigate to **Solutions** > **Information Protection** > **Classifiers** > **EDM classifiers**.
    
2.  Make sure the **New EDM experience** toggle is set to **Off**.
    
    [![Screenshot showing the toggle to use the new EDM experience.](../../wwl-sci/create-manage-sensitive-information-types/media/new-edm-toggle-off.png)](../../wwl-sci/create-manage-sensitive-information-types/media/new-edm-toggle-off.png#lightbox)
    
3.  Ensure the tab for **EDM schemas** is selected, then select **Create EDM schema** to open the schema tool configuration flyout.
    
4.  In the **New EDM schema** flyout panel, fill in an appropriate **Name** and **Description**.
    
5.  In the flyout panel, select **Ignore delimiters and punctuation for all schema fields** if you want to apply the **Ignore...** behavior for the entire schema.
    
6.  In the flyout panel, fill in your desired values for your **Schema field #1**. Add more fields as needed. Each schema field must be identical to the column headers in your sensitive information source file.
    
7.  In the flyout panel, if necessary, set the per-field values for the following:
    
    *   **Field is searchable**
    *   **Field is case-insensitive**
    *   **Choose delimiters and punctuation to ignore for this field**
    *   **Enter custom delimiters and punctuation for this field**
8.  Select **Save** at the bottom of the flyout panel. Your schema is now listed and available for use.
    
    [![Screenshot showing the new schema configuration options for EDM classic.](../../wwl-sci/create-manage-sensitive-information-types/media/edm-classic-schema-configuration.png)](../../wwl-sci/create-manage-sensitive-information-types/media/edm-classic-schema-configuration.png#lightbox)
    
9.  Back on the EDM classifiers page, select the tab for **EDM sensitive info types**.
    
10.  Select **\+ Create EDM sensitive info type** to start the **EDM rule package** wizard.
     
11.  On the **Define data store schema** page, **Choose an existing EDM schema** or select **New EDM schema** to create a new one.
     
12.  Select **Next**.
     
13.  On the **Define patterns for this EDM sensitive info type** page, select **Create pattern**.
     
14.  A flyout panel appears to define your new pattern. Choose the default confidence level for the pattern. The values are **Low confidence**, **Medium confidence**, and **High confidence**.
     
15.  In the flyout panel, select the **Primary element** for your EDM SIT. The options available for the primary element are based on fields you selected as searchable when you created your EDM schema.
     
16.  In the flyout panel, select an appropriate sensitive info type for your primary element.
     
17.  (Optional) In the flyout panel, add supporting elements if your data needs extra context to strengthen detection. The options available for the supporting element are based on the available fields that aren't your primary element.
     
18.  In the flyout panel, choose the **Matching options for supporting elements**. If multiple supporting elements are specified, select whether all or any must match. For Match if any, set the minimum and maximum number of matches (prefilled with 1 and the total number of elements).
     
19.  Select **Done** in the flyout panel.
     
20.  **Define patterns for this EDM sensitive info type**, then select **Next**.
     
21.  Choose the **recommended confidence level and character proximity** for this EDM SIT, then select **Next**.
     
22.  **Name and describe your EDM sensitive info type**, then select **Next**.
     
23.  **Review and finish** creating your EDM SIT by selecting **Submit**.
