...
url: "https://learn.microsoft.com/en-us/training/modules/purview-protect-on-premises-data/data-loss-prevention-on-premises"
...
2.  Create a DLP policy that includes the **On-premises repositories** location.
3.  Scope the policy to specific file paths if needed using inclusion or exclusion lists. Wildcards are supported in limited patterns.
4.  Run the policy in simulation mode first to validate its behavior before switching to enforcement.
5.  Monitor results in **Activity explorer**, the **Audit log**, or directly from scan reports on the scanner server.
