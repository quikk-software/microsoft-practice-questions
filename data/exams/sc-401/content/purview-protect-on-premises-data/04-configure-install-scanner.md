...
url: "https://learn.microsoft.com/en-us/training/modules/purview-protect-on-premises-data/configure-install-scanner"
...
3.  Give your cluster a **name** and optional **description**.
...
2.  Configure the scan job settings:
    ...
    *   **Preserve metadata**: Keeps original file attributes such as Date modified, Last modified, and Modified by during scanning. This setting is On by default.
        ...
        [![Screenshot showing where to edit a content scan job for the Microsoft Purview Information Protection scanner.](../../wwl-sci/purview-protect-on-premises-data/media/content-scan-job.png)](../../wwl-sci/purview-protect-on-premises-data/media/content-scan-job.png#lightbox)
        
3.  Add repositories on the **Repositories** tab:
    ...
    *   To scan an entire SharePoint root site, for example `http://sp2016`, the scanner account must have **Site Collection Auditor** permissions
        ...
        [![Screenshot showing the repository settings for the Microsoft Purview Information Protection scanner.](../../wwl-sci/purview-protect-on-premises-data/media/repository-settings.png)](../../wwl-sci/purview-protect-on-premises-data/media/repository-settings.png#lightbox)
...
2.  Select the scan job you created earlier.
3.  Change the **Schedule** setting from **Manual** to **Always** so that the scan runs continuously.
4.  Turn on **Enforce sensitivity labeling policy**. This allows the scanner to apply labels and protection automatically based on the policy.
5.  Once a scanner node is online, select **Scan now** to begin the job.
