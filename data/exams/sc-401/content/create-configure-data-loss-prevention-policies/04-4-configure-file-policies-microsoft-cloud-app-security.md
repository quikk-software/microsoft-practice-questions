---
title: "Configure policies in Microsoft Defender for Cloud Apps"
url: "https://learn.microsoft.com/en-us/training/modules/create-configure-data-loss-prevention-policies/4-configure-file-policies-microsoft-cloud-app-security"
uid: "learn.wwl.create-configure-data-loss-prevention-policies.configure-file-policies-microsoft-cloud-app-security"
...
2.  Select the **Information Protection** tab.
3.  Select **Create policy** and select **File policy**.
4.  Give your policy a name and description.
5.  Assign a **Policy severity**. The **Policy severity** setting determines whether matches trigger notifications based on your configured severity levels.
6.  Within **Category**, select the most appropriate risk type. The **Category** field helps you filter policies and alerts later. File policies default to DLP.
7.  **Create a filter for the files this policy will act on.** Be as restrictive as possible to avoid false positives. For example, if you want to target publicly shared files, add the **Public** filter. To target files shared with external users, use the **External** filter.
8.  Under **Apply to**, choose whether the policy covers all files or only specific folders within your connected apps. Scoping to specific folders reduces noise and focuses the policy on high-risk locations.
9.  Under **Select user groups**, choose which file owners the policy applies to. You can include all owners, specific groups, or exclude groups.
10.  Select the content **Inspection method**. You can select either [Built-in DLP](/en-us/defender-cloud-apps/content-inspection-built-in) or [Data Classification Services](/en-us/defender-cloud-apps/content-inspection?azure-portal=true). The Data Classification Service is recommended for a consistent experience across your DLP policies.
11.  Leave **Governance** actions unset for now and select **Create**.
...
The Inspection method supports built-in sensitive information types and custom expressions including substrings, exact string matches, and regular expressions. For more information, see [Working with the RegEx engine](/en-us/defender-cloud-apps/working-with-the-regex-engine?azure-portal=true).
...
2.  Find the file policy you want to review.
    
3.  In the **Count** column, select the number of **matches** for the policy. You can also select the three vertical dots (**…**) on the right side of the policy and select **View all matches**.
    
4.  The report opens with tabs that help you review results:
